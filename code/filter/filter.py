...
def file2list(file_name, as_float=False):
    ...
    print(f'finish process for {file_name}, has {len(line_list)} lines')
    if as_float:
...
def main():
    ...
    if args.output_mode == "classification":
        # pred (#examples, prob for being 0, prob for being 1), use the second prob
        list2file(preds[:, 1], os.path.join(args.output_dir, "score.txt"))
    else:
        # pred (#examples, 1)
        list2file(preds, os.path.join(args.output_dir,"score.txt"))
